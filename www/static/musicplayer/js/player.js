...
class Player {
    constructor(songs) { //类的构造函数
        //如果类已经有实例了，就返回这个实例
        if (Player.instance) return Player.instance;
        //如果没有实例化，就去构造一个实例
        return this.getInstance(songs);
    }

    //构建实例
    getInstance(songs) {
        let instance = new PlayerCreator(songs);
    ...
    constructor(songs) {
        if(songs == null){
            this.songs = [{
                    ...
                    songUrl: '../static/musicplayer/songs/song.mp3',
                    imageUrl: '../static/musicplayer/images/songs/c.jpg'
                },
                {
                    id: 2,
                    title: '认真的雪',
                    singer: '薛之谦',
                    songUrl: '../static/xmmusic/src/周杰伦 - 等你下课 (with 杨瑞代).flac',
                    imageUrl: '../static/musicplayer/images/songs/renzhendexue.jpg'
                },
                {
                    id: 3,
                    title: '演员',
                    singer: '薛之谦',
                    songUrl: '../static/xmmusic/src/周杰伦 - 简单爱.wav',
                    imageUrl: '../static/musicplayer/images/songs/yanyuan.jpg'
                }
            ]
        }else{
           this.songs = songs;
...
class PlayerCreator {
    ...
    constructor(songs) {
        ...
        //工具
        this.util = new Util();
        this.musics = new Musics(songs); //歌曲信息
